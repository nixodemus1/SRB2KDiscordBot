...
DiscordBot.Functions.statsofserver = function()
	...
	for player in players.iterate do
		if player
			...
			if player.realtime == nil then player.realtime = 0 end
			local miliseconds = G_TicsToCentiseconds(player.realtime)
			if string.len(miliseconds) == 1 then miliseconds = "0"..tostring(miliseconds) end
			local seconds = G_TicsToSeconds(player.realtime)
			if string.len(seconds) == 1 then seconds = "0"..tostring(seconds) end
			local pptime = G_TicsToMinutes(player.realtime, true)..":"..seconds.."."..miliseconds
...
COM_AddCommand("server_log", function(player, arg, text)
	...
	if arg == "msg"
		if DiscordBot.Data.msgsrb2
			local logmsg = assert(io.open("./client/DiscordBot/Messages.txt", "a+"))
			logmsg:write(DiscordBot.Data.msgsrb2)
			logmsg:close()
		end
	elseif arg == "logcom"
		if DiscordBot.Data.log
			local logcom = assert(io.open("./client/DiscordBot/logcom.txt", "a+"))
			logcom:write(DiscordBot.Data.log)
			logcom:close()
		end
	elseif arg == "pause"
		if DiscordBot.Data.pcmtsrb2
			local logmsg = assert(io.open("./client/DiscordBot/Players.txt", "w"))
	...
	elseif arg == "players"
		...
		if DiscordBot.Data.stats
			...
			local logmsg = assert(io.open("./client/DiscordBot/Stats.txt", "w"))
			logmsg:write(cv_servername.string.."\n"..DiscordBot.Data.maptitle.." ("..gamemap..")\n"..gamemap.."\n"..DiscordBot.Data.nextlevel.."\n"..DiscordBot.Data.iconemeralds.."\n"..playerskins.."\n"..ltime.."\n"..stime.."\n"..DiscordBot.Data.pcmp.."\n"..DiscordBot.Data.stats)
			logmsg:close()
		end
	elseif arg == "console"
		if DiscordBot.Data.console
			local d_console = assert(io.open("./client/DiscordBot/console.txt", "r"))
			if d_console
				local clear = false
				while true do
					local line = ''
				...
				d_console:close()
				if clear == true
					local d_console = assert(io.open("./client/DiscordBot/console.txt", "w"))
					d_console:write("")
					d_console:close()
				end
			end
	...
	elseif arg == "discord"
		local d_msg = assert(io.open("./client/DiscordBot/discordmessage.txt", "r"))
		if d_msg
			local d_msgread = nil
			local partmsg = ''
			...
			if d_msgread != ""
				...
				local d_msg = assert(io.open("./client/DiscordBot/discordmessage.txt", "w"))
				if partmsg == ''
					d_msg:write("")
				else
					d_msg:write("\""..partmsg.."\"")
